<?php
// api/base.php - Common base for KodeWeb IDE APIs
...
function connect_ftp($connInfo) {
    ...
    if (!$ftp) {
        ...
        @file_put_contents($GLOBALS['rootDir'] . '/data/ftp_error.log', date('Y-m-d H:i:s') . " - Connect Error ($host:$port): $msg\n", FILE_APPEND);
    ...
    if (!@ftp_login($ftp, $username, $password)) {
        $error = error_get_last();
        $msg = $error ? $error['message'] : "Desconhecido";
        @file_put_contents($GLOBALS['rootDir'] . '/data/ftp_error.log', date('Y-m-d H:i:s') . " - Login Error ($username@$host): $msg\n", FILE_APPEND);
